...
2.times do 
Event.all.each do |past_event|
    group = past_event.group
    ...
    event.tsar_id = users.sample.id
        if group.repeats_every == "week"
            event.watch_date = past_event.watch_date + 7.days
        elsif group.repeats_every == "two_weeks"
            event.watch_date = past_event.watch_date + 14.days
        elsif group.repeats_every == "month"
            event.watch_date = past_event.watch_date + 1.month
        elsif group.repeats_every == "six_months"
            event.watch_date = past_event.watch_date + 6.month
        elsif group.repeats_every == "year"
            event.watch_date = past_event.watch_date + 1.year
        end
    event.save 
